module tpgen(bfm_if bfm);
    ...
    task automatic print_routing_table();
        ...
        $display("\n%s==============================", esc_yellow);
        $display("   ROUTING TABLE DUMP (%0d entries)", bfm.routing_table.size());
        $display("==============================%s", esc_reset);
        $display(" Index | Address | Port ");
        $display("--------+----------+------");

        foreach (bfm.routing_table[i]) begin
            string port_str;
            if (bfm.routing_table[i].port == 0)
                port_str = {esc_green, "sout0", esc_reset};
            else if (bfm.routing_table[i].port == 1)
                port_str = {esc_green, "sout1", esc_reset};
            else
                port_str = {esc_yellow, "???", esc_reset};

            $display("  %3d   |  0x%02h    |  %s",
                     i, bfm.routing_table[i].addr, port_str);
        end
